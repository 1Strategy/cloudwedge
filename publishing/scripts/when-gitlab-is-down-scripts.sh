# if gitlab is down, this can be run to deploy cloudvelum still
...
ARTIFACT_BUCKET_PREFIX=public/cloudvelum
...
for REGION in "${REGION_LIST[@]}"; do
    ...
    sam package --s3-bucket ${ARTIFACT_BUCKET}-${REGION} --s3-prefix $ARTIFACT_BUCKET_PREFIX/$VERSION --output-template-file cloudvelum-$VERSION.yaml --region $REGION
    ...
    aws s3 cp cloudvelum-$VERSION.yaml s3://${ARTIFACT_BUCKET}-${REGION}/$ARTIFACT_BUCKET_PREFIX/$VERSION/ --region $REGION --acl $ACL
    echo "${BLUE}Uploading spoke template file...${NOCOLOR}"
    aws s3 cp app/cloudvelum-spoke.yaml s3://${ARTIFACT_BUCKET}-${REGION}/$ARTIFACT_BUCKET_PREFIX/$VERSION/ --region $REGION --acl $ACL
